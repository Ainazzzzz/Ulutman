...
import { MODERATION_MEDIA, MODERATION_MEDIA_DATA } from '../../utils/constants/moderation';
...
const selectsConfig = [
   ...
   { label: 'status', options: [{ id: 'e3', value: 'status', label: 'Cтатус' }] },
];
...
const reducer = (state, action) => {
   switch (action.type) {
      ...
      case 'TOGGLE_WAITING_MODAL':
         return { ...state, [action.type === 'TOGGLE_DELETE_MODAL' ? 'deleteAllModal' : 'waitingModal']: !state[action.type === 'TOGGLE_DELETE_MODAL' ? 'deleteAllModal' : 'waitingModal'] };
      ...
      case 'SET_DATE_VALUES':
         return { ...state, inputValues: { ...state.inputValues, date: action.payload.date } };
      case 'RESET_FILTER':
         return initialState;
      default:
         return state;
...
export const MediaFilesModerationPage = () => {
   ...
   const headers = useMemo(() => getAdminTableHeaders(() => handleToggle('TOGGLE_WAITING_MODAL'), MODERATION_MEDIA), [handleToggle]);
